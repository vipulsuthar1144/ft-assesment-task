...
import { removeItemFromFaqslist } from "@store/slices/faqs.slice";

const Faqs = () => {
  const dispatch = useAppDispatch();
  const { isFaqsLoading, isFaqsCRUDLoading, isFaqsError, faqsList } =
  ...
  const handleGetAllItemsAPICall = () => {
    faqsList.length == 0 && dispatch(FaqsAPI.getAll());
  };
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
          <LoaderButton
            StartIcon={Plus}
            label="Add new faqs"
            className=" md:text-sm font-normal rounded-md"
            onClick={listenerOnAddButton}
          />
          <button
