...
export const ArticleService = {
  getAllWithPagination: async (signal: AbortSignal, filter: IQueryParams) => {
    return await apiInstance.get(
      parseQueryParams(ApiEndpoints.getArticle, filter),
      {
        signal,
      }
    );
  ...
  create: async (signal: AbortSignal, data: FormData) => {
    return await apiInstance.post(`${ApiEndpoints.createArticle}`, data, {
      signal,
      headers: {
  ...
  updateById: async (
    ...
    return await apiInstance.put(`${ApiEndpoints.updateArticle}/${id}`, data, {
      signal,
    });
  },
