...
export const ArticleAPI = {
  getAll: getAsyncThunk<
    IBaseSchema<IBasePaginationSchema<IArticleSchema>>,
    IQueryParams
  >("ArticleAPI/getAll", async (filter, signal) => {
  ...
  updateById: getAsyncThunk<
    IBaseSchema<IArticleSchema>,
    { data: { title: string; description: string }; id: string }
  >("ArticleAPI/updateById", async ({ data, id }, signal) => {
